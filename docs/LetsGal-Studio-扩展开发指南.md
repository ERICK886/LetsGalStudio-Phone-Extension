...
| 能力 | 说明 | 典型用途 |
|------|------|---------|
| **可视化界面** | 用 22 种元素搭建 UI，不需要构建程序 | 标题、存读档、设置、菜单、HUD |
...
它保证每个项目开箱即可运行。默认游戏壳的主要界面也是可视化 UI，可以在当前项目中生成可编辑副本，或者绑定到你自己的界面。
...
展开扩展下的「界面」分组，点击新增按钮并输入显示名称。Studio 会创建界面文档并打开可视化编辑器。可以放置文字、图片、按钮、设置控件和存档格子，配置变量、点击动作与动画。

### 3.3 按需初始化程序

选择扩展下的「程序」，点击「初始化程序」后会补充以下文件结构：
...
| `category` | `string` | 分类（picker 里的分组） |
...
自定义程序 UI 应只提交可用选项的 `originalIndex`，然后关闭自身。
...
interface SystemInvokeOptions {
  ...
  containerOptions?: UIShowOptions;         // 可选的容器尺寸与位置
}
```
...
| 路径 | 说明 |
|------|------|
| **只做界面** | 新建扩展后直接创建 UI，不需要 npm 和代码 |
| **编写程序** | 初始化 TypeScript 工程，开发方法、逻辑或 React UI |
| **混合控制** | 给可视化元素设置引用名，再由程序动态控制 |
| **替换系统界面** | 把标题、存读档等位置绑定到自己的实现 |
...
> ⚠️ 彻底删除还会清理当前项目对该扩展的依赖关系。操作前先提交源码或制作备份。

### 15.4 项目发行物

扩展被项目启用后，Studio 会把运行所需的清单、界面和程序构建产物保存到项目内，形成一份**项目发行物**。预览和构建游戏时会使用这份发行物。

项目发行物会随项目目录一起复制和分享，但**不会包含 `src/` 等扩展源码**。这样，其他创作者即使没有安装原扩展，也可以正常打开、预览和构建项目。
...
| 文档 | 链接 |
|------|------|
| 扩展是什么 | https://docs.avg-engine.com/extensions/intro |
...
> **文档说明**：本指南基于 LetsGal Studio v1.8.0 官方文档整理编写，并整合了 `@avg-studio/sdk` 源码（`sdk-context.ts`、`extension-base.ts`、`extension-module.ts`、`save-schema.ts`、`settings-builder.ts`、`schedule-strategy.ts`、`types/` 目录下全部类型定义等）中的完整 API 签名与实现细节，补充了官方文档未覆盖的接口方法、类型定义和工具函数。供扩展开发工程师参考使用。如需了解最新更新，请访问[官方更新日志](https://avg-engine.com/changelog)。
