...
| 显示手机 HUD 按钮 | 挂载手机后显示触屏入口；关闭后仍可继续使用键盘快捷键 |
| 手机 HUD 按钮类型 | 支持图标按钮、文本按钮和铺满素材的图片按钮 |
| 手机 HUD 图标 | 提供手机、应用网格、菜单、消息、设置、星标预设，也可选择项目图片素材 |
| 手机 HUD 外观 | 提供深色玻璃、浅色、蓝色强调、透明和自定义颜色预设；可替换按钮背景图、背景色、内容色、边框与圆角 |
| 手机 HUD 按钮位置 | 支持左上、上中、右上、左中、右中、左下、下中、右下 |
| 手机 HUD 横向/纵向微调 | 在锚点基础上按像素调整位置；正值向右/向下移动 |
| 手机 HUD 按钮与内容大小 | 高度 24–240 像素；宽度可自动或自定义，图标/文字大小可独立调整 |
...
若项目曾启用独立聊天 / 相册扩展，请把「动作 · 手机内部应用」里的 Phone SDK 应用 ID 改为下表新值，并**停用**旧独立包（仅保留本扩展）：
...
在扩展设置的「手机电话 → 来电铃声」中可以选择一个音频素材；留空则不播放铃声。同一配置也会出现在「电话 APP → 来电界面」编辑页中，并提供素材 URI 与试听控件。

强制来电出现后，铃声会作为音效循环播放；接听、挂断、来电取消或手机界面关闭时都会自动停止。

## 5. 剧情里安装 / 禁用 APP，以及 Toast
