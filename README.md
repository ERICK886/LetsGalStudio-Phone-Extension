...
> 扩展版本：`1.2.5` ｜ 需要 LetsGal Studio SDK：`>=1.9.0`  
> 对齐：`@ink-zenly/phone-sdk@^0.5.3` ｜ `@ink-zenly/create-phone-app@0.3.6`

这是一个**游戏里的手机**：剧情里挂上之后，玩家可以打开桌面、点 APP、看聊天消息与相册，还能收到简单的提示（Toast）。
...
内置相册内页程序 ID：`phone-album`（桌面 APP 的 `phoneAppId` 填这个）。
...
10. [手机相册内页（phone-album）](#10-手机相册内页phone-album)
11. [用哪种 Preview、改完没生效怎么办](#11-用哪种-preview改完没生效怎么办)
12. [自检清单与常见问题](#12-自检清单与常见问题)
13. [更新日志](#13-更新日志)
...
- 内置**相册**内页：多相册、图片 / 视频浏览；视频可封面缩略与手机风播放
...
| 内容 | 可多行；空内容会跳过 | 同上 |
| 发送方 | 对方在左 / 我方在右 | 同上 |
| 消息状态 | 默认已读；对方除「撤回」外永远显示已读 | 同上 |
| 被拉黑提示 | 只有我方状态是「被拉黑」时才显示 | 同上 |
| 撤回延迟（毫秒） | 仅状态为「撤回」时生效；默认 `3000` | 同上 |
| 撤回后缀文案 | 仅「撤回」时生效；默认「撤回了一条消息」；最终显示为「角色名+后缀」 | 同上 |
...
- **撤回（双方）**：先正常出气泡，默认约 3 秒后气泡动态消失，变成居中灰字「{角色名}撤回了一条消息」（后缀可改）。角色名取该条聊天角色预设绑定的角色。倒计时未结束时玩家点推进：立刻变成撤回系统行，再继续下一条。
...
## 10. 手机相册内页（phone-album）

本扩展同包内置相册程序（`@extension({ id: "phone-album" })`）。桌面 APP 或动作里 **手机内部应用 ID** 填 `phone-album` 即可打开。

### 玩家能看到什么

- **首页**：虚拟「全部」+ 各相册卡片（封面、名称、数量）
- **网格**：图片缩略；视频优先封面图 → 再尝试抽首帧 → 失败则 ▶ 占位，并显示角标
- **查看器**：图片可左右滑切；视频为手机风控件（播放 / 暂停、进度、时长、静音；点画面显隐控件）
- **页面切换**：首页 ↔ 网格 ↔ 查看器 带滑动 / 淡入淡出过渡；顶栏返回逐级退出，首页再返回则回手机桌面
...
| 区域 | 作用 |
| --- | --- |
| 应用标题 / 「全部」显示名 / 空相册提示 | 文案 |
| 默认相册 | `id`、名称、可选封面图 |
| 默认媒体 | `id`、类型 `image` \| `video`、素材、所属相册 ID（逗号分隔）、可选视频时长秒、可选**视频封面图** |

剧情方法（模块名一般为「手机相册」）：

| 方法 | 作用 |
| --- | --- |
| 相册 · 新增相册 | 新增 / 覆盖相册（可选封面媒体或封面图） |
| 相册 · 移除相册 | 从列表隐藏（不删媒体） |
| 相册 · 新增媒体 | 新增 / 覆盖媒体；视频可填封面图与时长 |
| 相册 · 移除媒体 | 隐藏媒体 |
| 相册 · 设置媒体归属 | 替换媒体所属相册列表 |

快进 / 跳过仍会写存档，但不会打开 UI。玩家不能在相册里上传或删除。

### 视频小提示

- 建议为视频填「视频封面图」，网格最稳  
- 未填封面时会尝试 preload 视频首帧；Studio `local://` 下不强行 seek，避免误报「无法加载」  
- 播放依赖宿主 / 浏览器对素材格式的支持（常见 mp4 等）

## 11. 用哪种 Preview、改完没生效怎么办
...
- [ ] 个性化开关和「锁定编辑」符合预期  
- [ ] 消息能一条条推进，接续和背景符合预期  
- [ ] 相册能打开（`phoneAppId=phone-album`），图片 / 视频可浏览  
- [ ] 用剧本 Preview 能完整跑通  
...
| 相册打不开 | 桌面 APP 的内部应用 ID 是否为 `phone-album`；是否已挂载手机 |
| 视频格「无法加载」 | 建议填视频封面图；已重载最新扩展后仍失败则查素材格式 / 控制台 `[phone-album]` |
...
| 版本 | 主要变化 |
|------|------|
| **1.2.5**（当前） | 文档补齐相册；相册内页页面过渡（前进/后退滑动、查看器淡入淡出）；视频缩略不再错误 seek |
| **1.2.4** | 相册视频：封面/抽帧缩略 + 手机风播放器 |
| **1.2.3** | phone-sdk `0.5.3`：`show-message`「撤回」状态（延迟消失 + 系统行） |
| **1.2.2** | phone-sdk `0.5.2`：撤回能力首发 |
| **1.2.1** | phone-sdk `0.5.1`（`closePhoneApp`）；CLI `0.3.6`；内置 `phone-album` |
| **1.2.0** | 内置 `phone-album` 相册内页；phone-sdk `0.5.0`（`openPhoneApp` 深开内页）；CLI `0.3.5` |
| **1.1.1** | 可选：对方要说话前，先把我方「未读」点成「已读」（再点才出对方消息）；改已读时不再重播飞入动画 |
