...
本文档以当前源码与 `extension.json` 为准。`docs/手机UI插件方案.md` 是历史设计记录；其中的主题 ID、完整 CSS 编辑器等能力尚未实现。
...
| 添加或删除手机 APP | `manage-installed-apps` | 安装或删除已配置 APP。 | 同样修改 shared APP 状态。 |
| 禁用或解禁手机 APP | `manage-app-enabled-state` | 禁用或解禁已配置 APP。 | 同样修改 shared APP 状态。 |
...
| 设置 | 作用 |
| --- | --- |
| 手机标题、手机样式预设、手机弹出位置 | 样式预设可选苹果手机（默认）或安卓手机，切换外壳、顶部开孔/听筒、圆角与应用/消息圆角；弹出位置可选左上、中上、右上、左下、中下、右下或中部。 |
| 默认背景色 / 图 / CSS | 作者默认外观；玩家没有覆盖时生效。样式预设不会覆盖这些颜色与背景。 |
...
| 手机应用目录 | 应用 ID、名称、图标、默认排序、游戏开始默认预装、作者默认可用、锁定玩家编辑、默认动作 ID。 |
...
### 应用默认排序
Studio 的“手机应用目录”当前不支持扩展声明作者端的上下拖拽控件，因此每一行使用“默认排序”作为回退排序字段。它只接受 `0`～`9999` 的整数，数值越小越靠前；数值相同则保持应用在设置数组中的行顺序，旧项目缺少该字段时等同于 `0`。应用顺序仅由作者设置控制，剧情安装/解禁后仍按此顺序显示；玩家不能在手机桌面拖动应用。

### 剧情管理已配置 APP
在“手机应用目录”的每一行中，“游戏开始默认预装”决定新游戏是否拥有该 APP，“作者默认可用”决定它是否初始显示；旧项目未配置前者时默认视为已预装。剧情可使用两个方法覆盖这两个状态，覆盖会写入独立的 shared 状态，不受“允许玩家个性化手机”开关影响：

| 方法 | 操作 | 结果 |
| --- | --- | --- |
| 添加或删除手机 APP | 添加到手机 / 从手机删除 | 控制 APP 是否已安装；删除后不显示，但不会清除其玩家端名称、图标或动作偏好。 |
| 禁用或解禁手机 APP | 禁用 APP / 解禁 APP | 控制已安装 APP 是否可见；禁用不等同删除，解禁也不会自动重新安装已删除 APP。 |

每个 APP 管理块可填写第 1～8 个 APP ID。当前 SDK 的方法 schema 不支持把“手机应用目录”动态变成多选下拉，因此填写目录中的稳定应用 ID；运行时会验证 ID，只应用已配置 APP 并在 `[phone-debug] app-availability-updated` 中记录被忽略的 ID。APP 管理方法可在手机未挂载时执行，效果会在下次打开手机时体现。
...
组级字段为“接续上一组消息”（`appendToExisting`，默认否）、“本组结束后关闭手机”（`closeAfterMessages`，默认是）、“手机消息显示位置”（`popupPosition`，默认右下）和“聊天手机背景图”（`storyBackground`，可选图片素材）。位置只影响当前消息会话；普通手机位置由扩展设置控制。`storyBackground` 仅在消息模式中覆盖手机屏幕背景，不写入玩家 shared 存档，也不会改动普通手机背景；留空会立即恢复作者默认背景或当前生效的玩家背景/安全 CSS。接续消息组也以**当前方法块**的设置为准：新图会替换上一组背景，留空则恢复默认背景。
...
对方已读标记显示在气泡右下角外侧。消息模式只在手机**屏幕内部**叠加 `rgba(0, 0, 0, 0.4)` 黑色遮罩，不影响手机外部游戏画面；自定义聊天背景同样会保留该遮罩。第一条立即显示；之后每次点击手机消息区域或按 `Enter`/`Space` 追加一条。最后一条显示后还要再确认一次：关闭组会播放关闭动画并继续剧情；非关闭组会保留列表、进入下一个剧情块。`Escape` 也会关闭当前消息手机并结束等待。
...
如果程序预览仍是黑色面板，先执行 `pnpm run build`，再重载扩展或重启 Preview。特别是右侧方法说明仍显示旧版 `characterId`、`portraitId` 等字段时，说明 Studio 还缓存旧静态 schema；当前表单应显示 `presetId`、`message`、`direction`、`status`、`blockedHint` 和 `storyBackground`。重载前不要根据旧表单配置新剧情。
