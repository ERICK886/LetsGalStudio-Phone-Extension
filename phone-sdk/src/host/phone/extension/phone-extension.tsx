...
export class PhoneExtension extends Extension<PhoneUIProps> {
  ...
  static settings = settings((s) => ({
    ...
    chatRolePresets: s
      ...
      .describe(
        "每条预设绑定一个项目资产角色。消息块填写预设 ID。「显示头像 / 显示名称」默认开启；show-message 可用「跟随预设 / 显示 / 隐藏」按条覆盖，方法优先。只有选择“扩展素材库”时，才需要填写上方素材库的素材 ID。气泡样式五字段（字体大小 / 文字颜色 / 名称颜色 / 对话框颜色 / 自定义 CSS）均可空：未填使用默认外观；自定义 CSS 为声明列表且优先于结构化颜色与字号。样式在 show-message 展开时写入消息快照。",
      ),
    programUiActions: s
      .array("动作 · 程序 UI", (item) => ({
